...
const ForMe = () => {
  ...
  async function fetchData() {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_PORT}/api/placements/${userid}`
      );
  ...
  async function fetchUserDetails() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_PORT}/api/user/${userid}`
      );
      const data = await response.json();
      setUserDetails(data);
    } catch (error) {
  ...
  async function onApply(record) {
    if (userDetails?.resumeurl) {
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_APP_PORT}/api/appliedplacements/${id}`,
          {
            method: "POST",
            headers: {
        ...
        if (response.status === 200) {
          ...
          // send notification
          fetch(
            `${import.meta.env.VITE_APP_PORT}/api/adminnotifications/${userDetails?.userid}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  const onOk = async () => {
    if (list.length > 0) {
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_APP_PORT}/api/user/resume/${id}`,
          {
            method: "POST",
            body: formData,
          }
      ...
      >
        ...
        <Button
          ...
          onClick={() => {
            ...
            if(userDetails.resumeurl)
              {
           const url = userDetails.resumeurl;
           const main = (url).substring(0,url.length-3);
           const last = "jpg"
           const mainlast = main.concat(last);
           console.log(mainlast);
           window.open(mainlast, "_blank");
         }
