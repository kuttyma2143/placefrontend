...
const ContextProvider = ({ children }) => {
  ...
  async function checkUser(signedInUser) {
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_PORT}/api/user/`, {
        method: "POST",
        headers: {
  ...
  const login = async (adminobj) => {
    try {
      if (adminobj) {
        const response = await fetch(`${import.meta.env.VITE_APP_PORT}/api/admin`, {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
  ...
  const logout = async () => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_APP_PORT}/api/admin/deletecookie`,
      {
        credentials: "include",
      }
    );
