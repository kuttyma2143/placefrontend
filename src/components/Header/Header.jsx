...
const Header = () => {
  ...
  async function fetchAdminData() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_PORT}/api/adminnotifications`
      );
  ...
  async function fetchUserData() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_PORT}/api/usernotifications/${currentUser}`
      );
      const data = await response.json();
      setNotify(data);
    } catch (error) {
  ...
  async function clearAdminNotifications() {
    try {
      await fetch(`${import.meta.env.VITE_APP_PORT}/api/adminnotifications`, {
        method: "DELETE",
      })
